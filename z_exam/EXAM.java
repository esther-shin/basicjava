...
public class EXAM {
	...
	//답:
	int numOfBucket =
 	numOfApples/sizeOfBucket + //답:(numOfApples%sizeOfBucket > 0 ? 1 : 0) ;
 	
 	[실행결과]13
 	
	*[3-3] 아래는 변수 의 값에 따라 num ‘양수 ’, '음수' ‘0’  을을 출력하는 코드이다 
	*삼항 연산자를 이용해서 에 알맞은 코드를 넣으시오 (1) .
	...
	public static void main(String[] args) {
	int num = 10;
	System.out.println(답:num > 0 ? "양수":(num < 0 ? "음수" : "0"));
	*
	*[ ] 실행결과-양수
	*
	*
	*[3-4] 아래는 변수 num의 값 중에서 백의 자리 이하를 버리는 코드이다. 만일 변수 num
	...
	public static void main(String[] args) {
	int num = 333;
	System.out.println( 답: num/10*10+1);
	...
	public static void main(String[] args) {
	int num = 24;
	System.out.println(답: 10 - num%10);
	...
	public static void main(String[] args) {
	int fahrenheit = 100;
	float celcius = //답: (int)((5/9f * (fahrenheit - 32))*100 + 0.5) / 100f>;
	...
	ch = //답" (char)(ch + 2);
	float f = 3 /  //답:2f;
	long l = 3000 * 3000 * //답:  3000L;
	float f2 = 0.1f;
	double d = 0.1;
	boolean result =  //답(float)d==f2;
	System.out.println("c="+c);
	...
	public static void main(String[] args) {
	char ch = 'z';
	boolean b = // 답: ('a' <= ch && ch <= 'z') || ('A' <= ch && ch <= 'Z');
	...
	public static void main(String[] args) {
	char ch = 'A';
	char lowerCase = //답: ('A' <= ch && ch <= 'Z') ? (char)(ch+32): ch;
	System.out.println("ch:"+ch);
